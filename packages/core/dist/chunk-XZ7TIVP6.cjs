...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunkVMOZMJN5_cjs.MigrationService(c.env.DB);
      await migrationService.runPendingMigrations();
      console.log("[Bootstrap] Syncing collection configurations...");
      try {
        await chunkXJBWGGSD_cjs.syncCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing collections:", error);
      }
      console.log("[Bootstrap] Syncing form collections...");
      try {
        await chunkXJBWGGSD_cjs.syncAllFormCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing form collections:", error);
      }
      if (!config.plugins?.disableAll) {
        console.log("[Bootstrap] Bootstrapping core plugins...");
        const bootstrapService = new chunkXJBWGGSD_cjs.PluginBootstrapService(c.env.DB);
...
var metricsMiddleware = () => {
  return async (c, next) => {
    ...
    if (path !== "/admin/dashboard/api/metrics") {
      chunkRGFEUN6J_cjs.metricsTracker.recordRequest();
    }
    await next();
  };
};
...
//# sourceMappingURL=chunk-XZ7TIVP6.cjs.map
//# sourceMappingURL=chunk-XZ7TIVP6.cjs.map