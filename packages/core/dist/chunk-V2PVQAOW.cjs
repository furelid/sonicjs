...
var VALID_FILTER_OPERATORS = /* @__PURE__ */ new Set([
  "equals",
  "not_equals",
  "greater_than",
  "greater_than_equal",
...
var QueryFilterBuilder = class {
  ...
  build(baseTable, filter) {
    ...
    if (filter.sort && filter.sort.length > 0) {
      const orderClauses = filter.sort.map((s) => `${this.sanitizeFieldName(s.field)} ${this.sanitizeSortOrder(s.order)}`).join(", ");
  ...
  static parseFromQuery(query) {
    ...
    if (query.sort) {
      try {
        const parsed = typeof query.sort === "string" ? JSON.parse(query.sort) : query.sort;
        if (Array.isArray(parsed)) {
          filter.sort = parsed.filter((s) => s && typeof s.field === "string").map((s) => ({
            field: s.field,
...
var package_default = {
  ...
  scripts: {
    "generate:migrations": "pnpm dlx tsx scripts/generate-migrations.ts",
    prebuild: "pnpm run generate:migrations",
    build: "tsup",
    dev: "tsup --watch",
...
exports.templateRenderer = templateRenderer;
//# sourceMappingURL=chunk-V2PVQAOW.cjs.map
//# sourceMappingURL=chunk-V2PVQAOW.cjs.map