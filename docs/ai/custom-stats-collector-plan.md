...
- Track **local installs only** (via `pnpm dlx create-sonicjs`)
...
| package_manager | TEXT | pnpm, yarn, pnpm, bun |
...
1. **`installation_started`**
   - Fired when `pnpm dlx create-sonicjs` begins
   - Properties:
     - `os`: Platform (darwin, linux, win32)
     - `nodeVersion`: Node.js version
     - `packageManager`: pnpm, yarn, pnpm, bun
...
2. Run `pnpm create-sonicjs stats` to scaffold the app
...
1. Run `pnpm dlx create-sonicjs test-app`
