...
Read endpoints are public by default. Write endpoints require a JWT — see the [authentication guide](/authentication). For the full payload shapes, query parameters, and pagination metadata, the [API reference](/api) is the canonical source.
...
```bash
pnpm dlx create-next-app@latest my-next-site \
...
Read endpoints are public, but writes (create / update / delete) require a JWT. The safest pattern is a **Route Handler** that runs on the server, attaches the token, and proxies the request — your token never reaches the browser.
...
The same pattern works for `PATCH`, `DELETE`, file uploads to `/api/media`, and any other mutation endpoint. See the [SDKs page](/sdks) for typed clients that wrap these endpoints if you'd rather skip the manual `fetch`.
...
{
  "scripts": {
    "build:cf": "next build && pnpm dlx @cloudflare/next-on-pages",
